...
contract Ballot{
	...
	//This delcares a state variable that stores a "voter" struct for each possible address.
	...
	function Ballot(bytes32[] proposalNames){
		...
		//For each provided proposal names, create a new proposalobject and add it to the end of the array.
	...
	//Give "voter" the right to vote in this ballot.
	//May only be canceled by "chairperson".
	function giveRightToVote(address voter){
		if (msg.sender != chairperson || voters[voter].voted){
			//"throw terminates and reverts all changes to the state and to Ether balances.
			//It is often a good idea to use this if functions are called incorrectly.
	...
	function delegate (address to){
		...
		if (sender.voted)
			throw;
		//Forward the delegation ad long as "to" also delegated.
		...
		//In this case, the delegation will not be executed, but in other situations, such loops might cause contract to get stuck completely.
		...
		if (delegate.voted){
			...
			//If the delegate did nto vote yet, add to their weight.
